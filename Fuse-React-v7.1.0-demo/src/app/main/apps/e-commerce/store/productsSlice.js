...
export const getProducts = createAsyncThunk('eCommerceApp/products/getProducts', async () => {
  const response = await axios.get('https://dev.crystovajewels.com/api/v1/product/get');
...
export const removeProducts = createAsyncThunk(
  ...
  async (productIds, { dispatch, getState }) => {
    if (Array.isArray(productIds)) {
      ...
      await axios.delete('https://dev.crystovajewels.com/api/v1/product/multi-delete', {
        data: { ids: productIds }
      });
    } else {
      // Single delete API call
      await axios.delete(`https://dev.crystovajewels.com/api/v1/product/delete/${productIds}`);
    }
    return productIds;
  }
);
